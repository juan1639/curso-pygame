...
class Game:
	...
	def instancias(self):
		suelo = self.RESOLUCION[1] - self.TY
		RndX = self.RESOLUCION[0] // 2

		for i in range(4):
			plataforma = Plataforma(self, random.randrange(RndX), suelo - i * (self.TY * 3), i)
			self.lista_spritesAdibujar.add(plataforma)

		self.personaje = Personaje(self, self.RESOLUCION[0] // 2, self.RESOLUCION[1] - self.TY * 3)
